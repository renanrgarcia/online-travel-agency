...
- **It would contradict the reason this task exists.** `docs/reference/02-price-integrity.md`'s whole argument is
...
| E4 | `"This is {{PRICE_OFF1}}, about 20% cheaper."` | Rejected as a violation — `20` is a digit outside any token | Per `docs/reference/02-price-integrity.md`, **any** digit outside a token span is a violation. A model that computes its own comparison is exactly what this catches |
...
| E12 | `"Custa cerca de setecentos reais para a viagem."` (Portuguese, no digits, no tokens) | Rejected as a violation | The target market is Brazilian (see `docs/reference/03-suppliers-and-budget.md`); an English-only guard would be a product bug, not just an incomplete one |

### Locked decisions

- **The guard's rule is absolute: any digit character outside a token span is a violation.** Not
  "price-shaped numbers" — any digit. This is what `docs/reference/02-price-integrity.md` specifies, and it's
